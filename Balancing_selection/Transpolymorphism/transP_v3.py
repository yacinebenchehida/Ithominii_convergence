...
def extract_transpolymorphic_sites(vcf_file, pop_file, chromosome, output_file, working_dir):
    ...
    for record in vcf.fetch():
        ...
        if polymorphic_in_required_taxa and all(all(count >= 6 for count in allele_counts.values()) for allele_counts in species_allele_counts.values()):  # Change the value to set a different threshold 
