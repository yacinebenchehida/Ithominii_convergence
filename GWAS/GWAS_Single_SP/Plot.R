...
# Plot #
########
png(file="GWAS_zoom_optix.png",width=800,height=500,type="cairo")
ggplot(gwas_data[(gwas_data$bp_cum > 25900000) & (gwas_data$bp_cum < 26100000),], aes(x = bp_cum, y = -log10(P),
                                  color =CHR)) +
  geom_point(alpha = 0.75) +
  scale_x_continuous() +
  scale_color_manual(values = rep(c("blue4", "orange3"), unique(length(axis_set$CHR)))) +
