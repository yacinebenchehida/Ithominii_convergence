...
for (sp in dat){
	...
	geom_gene_arrow()  +
	scale_color_manual(values = c("black")) + scale_fill_manual(values = c("gold","azure3","coral","forestgreen","magenta1","black")) + guides(color = "none")+
	...
	######################################  
        ...
        ######################################
	png(file=paste(sp,"_Zoom_GWAS.png",sep=""),width=700,height=500)
